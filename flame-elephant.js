...
(function(root){
  ...
  function buyFlame(){togglePlacementPreview(TYPE,()=>state.status==='playing'&&state.currency>=definition.cost&&canReserveElephantType(TYPE))}
  flameUI.buy.addEventListener('click',buyFlame);
  const flameRefreshBase=refreshUnitButtons;refreshUnitButtons=function(){flameRefreshBase();const canPlace=canReserveElephantType(TYPE);flameUI.buy.disabled=state.currency<definition.cost||state.status!=='playing'||!canPlace;flameUI.buy.classList.toggle('selected',state.placement===TYPE);if(state.placement===TYPE){ui.hint.querySelector('strong').textContent='PLACE FLAME ELEPHANT';ui.hint.querySelector('p').lastChild.textContent=' Click valid ground. Burns orbs, melts Ice Shields, and cannot be frozen.'}};
  canvas.addEventListener('click',event=>{if(state.placement!==TYPE)return;event.stopImmediatePropagation();updatePlacementPointer(event);if(!placementAllowed(mouse)||state.currency<definition.cost||!canPlaceReservedElephant(TYPE))return;state.currency-=definition.cost;const tower=applyPlacementFacing(createFlameElephant(mouse.x,mouse.y));state.towers.push(tower);scanActiveFumeOrbs(true);state.placement=null;selectTower(tower);tutorialAction('placed');burst(tower.x,tower.y,'#ff762e',20,82);updateUI()},true);
