...
(function(root){
  ...
  const DEBUG_ICE_SHIELD_HITS=Math.max(1,Math.floor(Number(frozenMap?.iceShield?.shieldHits)||2));
  ...
  function markIceShieldMutation(orb,wave,rules=activeIceShieldRules,roll=gameRandom()){if(!orb||orb.iceShieldChecked)return !!orb?.iceShieldHits;orb.iceShieldChecked=true;if(rules&&wave>=rules.firstWave&&roll<rules.spawnChance){orb.iceShieldHits=rules.shieldHits;orb.iceShieldMaxHits=rules.shieldHits;recordIceShieldEncounter(orb)}return !!orb.iceShieldHits}
  ...
  function applySnowCapFrostHeal(orb){
    ...
    orb.snowWeightHeals=snowWeightHeals(orb)+1;orb.snowHealFlash=.56;orb.snowHealText={amount:heal,life:.72,max:.72};burst(orb.x,orb.y,'#c8f8ff',14,78);
    return heal;
  }
  ...
  // Frost Elephants are zero-damage slowing towers. Their frost cannot chip
  // an Ice Shield, apply slow, or activate Snow-Capped healing; only a valid
  // damaging hit is allowed to consume a shield layer.
  ...
  function markDebugSpawn(orb,wave,override){
    ...
    if(override==='iceshield'){orb.snowMutationChecked=true;orb.snowCovered=false;delete orb.snowFreezeTimer;orb.iceShieldChecked=true;orb.iceShieldHits=DEBUG_ICE_SHIELD_HITS;orb.iceShieldMaxHits=DEBUG_ICE_SHIELD_HITS;orb.iceShieldImpactLife=0;orb.iceShieldCrackLife=0;orb.iceShieldBroken=false;recordIceShieldEncounter(orb);return true}
  ...
  const iceShieldDictionaryBase=encyclopediaEntries;encyclopediaEntries=function(){const entries=iceShieldDictionaryBase();if(!activeIceShieldRules||entries.some(entry=>entry.id==='mutation-ice-shield'))return entries;return entries.concat({id:'mutation-ice-shield',category:'special',tier:null,name:'Ice Shield Orb',description:'Blocks the next 2 elephant hits, regardless of damage.',introductionWave:`Ice Map Wave ${activeIceShieldRules.firstWave}`,baseHP:'2 Shield Hits',scaling:'Each valid elephant attack removes exactly 1 hit',reward:'Base orb reward unchanged',endpointDamage:'Base orb type',baseSpeed:'Base orb type',speedScaling:'Base orb type',visual:{highlight:'#effcff',mid:'#9beeff',low:'#4c93b2',shadow:'#113447',rim:'#d9fbff',glow:'#64dfff'},specialDetails:['Blocks direct attacks, Splash pulses, Frost hits, and Mini Robot attacks','Frost does not slow or heal until the shield breaks','The second blocked hit shatters the shell permanently']})};

  function updateSnowCapFrostVisuals(dt){for(const orb of state.balls){orb.snowHealFlash=Math.max(0,(orb.snowHealFlash||0)-dt);orb.iceShieldImpactLife=Math.max(0,(orb.iceShieldImpactLife||0)-dt);orb.iceShieldCrackLife=Math.max(0,(orb.iceShieldCrackLife||0)-dt);if(orb.snowHealText)orb.snowHealText.life=Math.max(0,orb.snowHealText.life-dt)}}
  ...
  function drawSnowCover(orb){
    ...
    if(orb.snowHealText?.life>0){const note=orb.snowHealText,progress=1-note.life/note.max;ctx.save();ctx.globalAlpha=Math.min(1,note.life/.16);ctx.fillStyle='#91efaa';ctx.shadowColor='#133d23';ctx.shadowBlur=4;ctx.font='900 12px Inter';ctx.textAlign='center';ctx.fillText(`+${Math.round(note.amount)}`,orb.x,orb.y-31-progress*20);ctx.textAlign='left';ctx.restore()}
  ...
  function drawFrostSlowStatus(orb){
    const stacks=Math.max(0,Math.min(CONFIG.FROST_SLOW_MAX_STACKS,activeFrostSlowStacks(orb)));if(!stacks)return;
    ctx.save();ctx.translate(orb.x,orb.y);ctx.globalAlpha=.16+stacks*.09;ctx.fillStyle='#7cecff';ctx.shadowColor='#77eaff';ctx.shadowBlur=6+stacks*4;ctx.beginPath();ctx.arc(0,0,BALL_R+stacks*2,0,Math.PI*2);ctx.fill();ctx.globalAlpha=.55+.08*stacks;ctx.strokeStyle='#d4fbff';ctx.lineWidth=1.2+stacks*.35;ctx.beginPath();ctx.arc(0,0,BALL_R+2+stacks*2,0,Math.PI*2);ctx.stroke();
    if(!prefersReducedMotion())for(let index=0;index<stacks+1;index++){const angle=state.gameTime*3+index*Math.PI*2/(stacks+1),distance=BALL_R+7+stacks*1.5;ctx.fillStyle='#efffff';ctx.beginPath();ctx.arc(Math.cos(angle)*distance,Math.sin(angle)*distance,1.4+stacks*.14,0,Math.PI*2);ctx.fill()}
    ctx.restore();
  }
  function drawIceShield(orb){
    const hits=iceShieldHits(orb),shatterLife=orb.iceShieldCrackLife||0,shattering=!hits&&shatterLife>0;if(!hits&&!shattering)return;
    if(hits){ctx.save();ctx.translate(orb.x,orb.y);ctx.beginPath();ctx.arc(0,0,BALL_R-1,0,Math.PI*2);ctx.clip();const shell=ctx.createRadialGradient(-7,-9,2,0,0,BALL_R);shell.addColorStop(0,'#f3ffff99');shell.addColorStop(.36,'#b8efff48');shell.addColorStop(.76,'#73cfe133');shell.addColorStop(1,'#d9fbff70');ctx.fillStyle=shell;ctx.fillRect(-BALL_R,-BALL_R,BALL_R*2,BALL_R*2);ctx.globalAlpha=.82;ctx.strokeStyle='#dffcff';ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,0,BALL_R-2,.48,2.48);ctx.stroke();ctx.globalAlpha=.62;ctx.strokeStyle='#f8ffff';ctx.lineWidth=2.5;ctx.beginPath();ctx.arc(-4,-4,BALL_R-7,3.7,5.1);ctx.stroke();ctx.lineWidth=1.25;ctx.strokeStyle='#9adff0';for(const angle of [-2.55,.28,2.18]){ctx.beginPath();ctx.arc(0,0,BALL_R-3,angle,angle+.38);ctx.stroke()}if(hits===1){ctx.globalAlpha=.8;ctx.strokeStyle='#dffcff';ctx.lineWidth=1.35;ctx.beginPath();ctx.moveTo(3,-16);ctx.lineTo(-1,-6);ctx.lineTo(5,-1);ctx.lineTo(1,10);ctx.moveTo(-1,-6);ctx.lineTo(-9,-2);ctx.stroke()}ctx.restore();ctx.save();ctx.translate(orb.x,orb.y);ctx.strokeStyle='#a9eefa';ctx.globalAlpha=.72;ctx.lineWidth=1.4;ctx.beginPath();ctx.arc(0,0,BALL_R-1,0,Math.PI*2);ctx.stroke();ctx.restore()}else{const progress=1-shatterLife/.62;ctx.save();ctx.translate(orb.x,orb.y);ctx.globalAlpha=Math.min(1,shatterLife/.16);ctx.fillStyle='#c6f7ff';for(let index=0;index<10;index++){const angle=index*Math.PI*2/10+.19,distance=BALL_R*(.35+progress*1.45),size=2.5+(index%3);ctx.save();ctx.translate(Math.cos(angle)*distance,Math.sin(angle)*distance);ctx.rotate(angle+progress*2);ctx.beginPath();ctx.moveTo(-size,-size*.45);ctx.lineTo(size,-size*.7);ctx.lineTo(size*.45,size);ctx.lineTo(-size*.7,size*.5);ctx.closePath();ctx.fill();ctx.restore()}ctx.restore()}
  }
  ...
  function renderFreezeResistance(){
    ...
    const heater=tower.towerType==='heater';
    if(heater){uiFreeze.panel.classList.remove('frozen');uiFreeze.panel.classList.add('heated');uiFreeze.panel.title='Heaters are permanent support structures and cannot freeze.';uiFreeze.label.textContent='Freeze Resistance: ∞ / ∞';uiFreeze.timer.textContent='';if(uiFreeze.status){uiFreeze.status.textContent='Permanent Heat Source';uiFreeze.status.classList.remove('hidden')}if(uiFreeze.fill)uiFreeze.fill.style.width='100%';if(uiFreeze.bar){uiFreeze.bar.setAttribute('aria-valuemax','1');uiFreeze.bar.setAttribute('aria-valuenow','1');uiFreeze.bar.setAttribute('aria-valuetext','Permanent heat source')}return}
