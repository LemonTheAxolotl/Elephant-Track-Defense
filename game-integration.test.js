...
assert.match(css,/\.freeze-resistance-segments/);
...
assert.equal(run('state.debugMessage'),'Spawned 1 engineer orb.');
...
assert.equal(run('state.debugMessage'),'Spawned 20 engineer orbs.');
...
assert.equal(run('state.debugMessage'),'Unknown mutation: nope. Available mutations: snowcap.');
...
assert.equal(run('ui.upgradePreview.textContent'),'Deploys Mini Robot Elephants from the track exit. Factories placed: 1 / 5 • Active Mini Robots: 0 / 50');
...
run('state.balls=state.balls.filter(orb=>!orb.bossReinforcement);updateBossPhaseShield(phaseBoss)');
...
assert.equal(run('state.balls.every(orb=>orb.snowMutationChecked&&orb.snowCovered===false)'),true);
...
assert.equal(run('state.debugMessage'),'Unknown mutation: blizzard. Available mutations: snowcap.');
...
assert.equal(run('state.debugMessage.includes("/spawn engineer [count] [mutation]")&&state.debugMessage.includes("Mutation available: snowcap")&&state.debugMessage.includes("/spawn normal 2 5 snowcap")'),true);
...
assert.equal(run('state.adminDebugMessage'),'Unknown mutation: freezer. Available mutations: snowcap.');
run('executeAdminDebugCommand("/help")');
assert.equal(run('state.adminDebugMessage.includes("/spawn normal <tier> [count] [mutation]")&&state.adminDebugMessage.includes("Mutation available: snowcap")&&state.adminDebugMessage.includes("/spawn normal 2 5 snowcap")'),true);
run('clearFinaleSave()');
