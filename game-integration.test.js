...
assert.equal(run('iceMapStartingCurrency'),125);
...
assert.deepEqual(JSON.parse(JSON.stringify(run('iceStartsByDifficulty'))),{easy:125,normal:125,hard:125});
assert.equal(run('iceRestartCurrency'),125);
assert.equal(run('state.currency'),100);
...
run('var adminOrbCount=state.balls.length;executeAdminDebugCommand("/kill all")');
