...
const DIFFICULTY_RULES={
  easy:{name:'Easy',hpMultiplier:.97,speedMultiplier:.98,damageMultiplier:1.05,description:'Easy — Orbs have 3% less HP • Move 2% slower • Elephants deal 5% more damage'},
  normal:{name:'Normal',hpMultiplier:1,damageMultiplier:1,description:'Normal — Standard game balance'},
  hard:{name:'Hard',hpMultiplier:1.03,speedMultiplier:1.02,damageMultiplier:.95,description:'Hard — Orbs have 3% more HP • Move 2% faster • Elephants deal 5% less damage'}
};
const DIFFICULTY_RUN_STORAGE_KEY='elephant-track-defense-difficulty-run',DIFFICULTY_RUN_VERSION=2;
...
function applyDifficultyToOrb(orb){
  if(!orb||orb.difficultyApplied)return orb;
...
orbMovementSpeed=function(orb){return difficultyOrbMovementSpeedBase(orb)*(orb?.isBoss?1:(difficultyRule().speedMultiplier??1))};
