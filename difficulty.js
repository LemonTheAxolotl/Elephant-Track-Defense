...
function grantHardFrozenStarterWaterGift(){
  if(state.hardFrozenStarterWaterGiftGranted)return false;
  state.freeStarterWaterElephantCount=1;state.hardFrozenStarterWaterGiftGranted=true;
  state.starterGiftGranted=true;state.starterGiftPending=true;state.starterGiftPlaced=false;state.starterGiftCost=0;
...
function setRunDifficulty(key){
  ...
  if(globalThis.GameContent.activeMap?.id==='frozen_expanse'&&key==='hard')grantHardFrozenStarterWaterGift();
