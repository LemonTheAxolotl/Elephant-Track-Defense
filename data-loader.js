(function(root){
  ...
  const VERSION='20260812-loader-2';
  const SCRIPTS=['orb-progression.js','game.js','difficulty.js','currency.js','recovery.js','engineer-dig.js','content-runtime.js'].map(path=>new URL(`${path}?v=${VERSION}`,document.baseURI).href);
  ...
  async function start(raw){
    if(!root.GameContentSchema)throw new Error('content-schema.js did not load');root.GameContent=root.GameContentSchema.build(raw);for(const script of SCRIPTS)await loadScript(script);
  }
  async function boot(){
    try{
