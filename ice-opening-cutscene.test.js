...
assert.equal(sandbox.ICE_OPENING_SCENES.length,29);
...
assert.ok(snowSuccessIndex>=0&&shieldSuccessIndex>=0&&refinementStartIndex>Math.max(snowSuccessIndex,shieldSuccessIndex)&&refinementFinalIndex>refinementStartIndex&&snowSetupIndex>refinementFinalIndex&&snowTestIndex>snowSetupIndex&&shieldTestIndex>snowTestIndex,'The Formula Refinement montage must finish before the restored Snowcap and Ice Shield outdoor tests.');
